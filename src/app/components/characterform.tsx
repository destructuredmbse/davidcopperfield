...
export default function StaffCardForm({ 
  ...
  const handlePlayedByChange = (actors: actor[]) => {
    setFormData(prev => {
      return {...prev,
      played_by: actors}
    });
    ...
    <div className={twMerge(className, 'p-1 z-20 w-48 h-96 border rounded-lg shadow-xl bg-white relative')}>
      <form onSubmit={handleSubmit} className="p-2 h-full flex flex-col">
        <div className="flex-1 space-y-2">
          ...
          <PlaybySelector handlePlayedByChange={handlePlayedByChange}/>
          </div>
        {/* Action Buttons */}
...
function PlaybySelector({ handlePlayedByChange } : { handlePlayedByChange: (value:actor[]) => void}) {
  ...
  return (
    <Combobox.Root items={cast} onValueChange={handlePlayedByChange} multiple>
